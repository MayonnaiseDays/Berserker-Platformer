/*public class ProjectileGeneric : MonoBehaviour
{
    ...
    public Vector2 direction = Vector2.right; // Direction the projectile will be launched in
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        if (collision.gameObject.CompareTag("Player") && !isShotByPlayer)
        {
            // If projectile is shot by enemy and hits the player
            HandlePlayerHit(collision.gameObject);
        }
        else if (collision.gameObject.CompareTag("Enemy") && isShotByPlayer)
        {
